...
try {
    ...
    // Get the Word file for this order
    $stmt = $pdo->prepare("
    ...
    if (!$file) {
        ...
        echo json_encode(['success' => false, 'error' => 'Word file not found for this order']);
        exit();
    }
    ...
    if (!file_exists($fullPath)) {
        http_response_code(404);
        echo json_encode(['success' => false, 'error' => 'Word file not found on server']);
        exit();
    }
    ...
    $filename = "Order_{$orderNumber}.docx";
    ...
    error_log("Error downloading order Word: " . $e->getMessage());
